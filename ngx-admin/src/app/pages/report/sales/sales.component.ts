...
export class SalesComponent implements OnInit {
  ...
  async saveNewContact(contact: EContact) {
    ...
    await this.invoice.changeContact(this.toast, this.selectedInvoiceId, contact.id)
  ...
  async submitSearch() {
    this.parameter.date.start = (this.parameter.date.start ? this.parameter.date.start : new Date())
    this.parameter.date.end = (this.parameter.date.end ? this.parameter.date.end : this.parameter.date.start)
    this.parameter.dateStart = Moment(this.parameter.date.start).format(Entity.MOMENT_PATTERN_MYSQL)
    this.parameter.dateEnd = Moment(this.parameter.date.end).format(Entity.MOMENT_PATTERN_MYSQL)
    this.parameter.useDateBetween = !!this.parameter.date.end
    this.result = await this.report.getReportSelling(this.toast, this.parameter)
  }
  ...
  async saveInvoice() {
    if (this.inv.isItemEmpty()) {
      ...
      this.toast.pop(Msg.error('', 'Mohon pilih customer terlebih dahulu untuk melengkapi data hutang/piutang!'))
    } else {

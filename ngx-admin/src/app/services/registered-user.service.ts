...
export class RegisteredUserService {
  ...
  update(form: FormData): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.http.post<any>(environment.api + '/user/profile', form)
        .subscribe(
          data => {
            console.log(data)
  ...
  async get(toaster: ToasterService, idUser: string): Promise<EUser> {
    ...
    this.loader.increase()
    try {
      let r = await this.http.get(environment.api + '/user/get/' + idUser).toPromise()
  ...
  list(limit: number, index: number, orderBy?: string, orderType?: string, query = ''): Promise<any> {
    this.loader.increase()
    return new Promise((resolve, reject) => {
      this.http.get<any>(environment.api + '/user/list/' + limit + '/' + index + '/' + orderBy + '/' + orderType + '?where=' + query)
        .subscribe(
          data => {
            resolve(data[ 'data' ])
          },
  ...
  toggleStatus(id: string): Promise<any> {
    this.loader.increase()
    return new Promise((resolve, reject) => {
      this.http.get<any>(environment.api + '/user/toggle-status/' + id)
        .subscribe(
          data => {
            this.loader.decrease()
            resolve(data[ 'data' ][ 'message' ])

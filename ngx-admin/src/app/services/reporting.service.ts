...
export class ReportingService {
  ...
  private async getReport(toast: ToasterService, param: any, type = 'selling'): Promise<Array<EReportTransaction>> {
    ...
    let arr = new Array()
    try {
      let r: any = await this.http.get<any>('api/reporting/' + type, { params: param }).toPromise()
      if (!!r && 'list' in r[ 'data' ]) {
  ...
  public async getReportStock(toast: ToasterService, param: any): Promise<Array<EReportStock>> {
    ...
    let arr = new Array()
    try {
      let r: any = await this.http.get<any>('api/reporting/stock', { params: JSON.parse(JSON.stringify(param)) }).toPromise()
      if (!!r && 'list' in r[ 'data' ]) {
        for (let i = 0; i < r[ 'data' ][ 'list' ].length; i++) {
  ...
  public async getReportStockDetails(toast: ToasterService, id: string, date: string): Promise<Array<EReportStockDetail>> {
    ...
    let arr = new Array()
    try {
      let r: any = await this.http.get<any>('api/reporting/stock-details', {
        params: {
          id: id,
          date: date,
        },
